...
/**
 * Makes the bot react to a particular message
 * @param bot_key Telegram Bot Key generated by BotFather !secret
 * @param chat_id Chat the Telegram Bot is in
 * @param message_id Message Id to react to
 * @param reaction Array of TeleReactions to apply. Note: Currently only allows one.
 * @param isBig Optional: True for a big animation to play on reaction
 */
  async setMessageReaction(
    chat_id: number | string,
    message_id: number,
    reaction: Array<TeleReaction>,
    isBig?: boolean,
  ) {
...
export async function setMessageReaction(
  ...
  return new Promise<TeleResponse>((resolve, reject) => {
    axios
      .post<TeleResponse>(`${TELE_API}${bot_key}/setMessageReaction`, {
        chat_id,
        message_id,
        reaction,
        isBig,
      ...
      .then((res) => {
        if (LOGGER == 'VERBOSE')
          if (res.data.result) {
            console.log(
              `Message successfully reacted to (chat_id: ${chat_id}, msg_id: ${message_id})`,
            )
          } else {
            console.log(
              `Message failed to be reacted to (chat_id: ${chat_id}, msg_id: ${message_id})`,
            )
          }
